...
import { ConfirmDeleteModalStudy } from "../confirm-delete-modal-study/ConfirmDeleteModalStudy";
...
export function StudyResultModal({study:initialStudy,open,onClose,onConfirm,idStudy,viewMode}:StudyResultModalProps){
    ...
    useEffect(()=>{
        ...
        if(initialStudy){
            ...
            setTitle('¿Estas seguro de que quieres editar este estudio?')
        }else if(idStudy === -1){
            setStudy(new Study())
            setStudyDate(null)
            setTitle('¿Estas seguro de que quieres crear este estudio?')
        }
